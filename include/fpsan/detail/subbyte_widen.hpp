...
namespace fpsan
{
    namespace detail
    {
        ...
        FPSAN_HOST_DEVICE constexpr AlgConfig finite_subbyte_alg_cfg()
        {
        ...
        template <int Width>
        FPSAN_HOST_DEVICE constexpr std::uint32_t subbyte_positive_max_code()
        {
            return (std::uint32_t{1} << (Width - 1)) - 1u;
        }

        template <int Width, Semantics S>
        FPSAN_HOST_DEVICE constexpr std::uint32_t
            canonical_subbyte_code_from_residue(std::uint64_t p)
        {
            static_assert(is_algebraic_semantics(S), "algebraic semantics required");
            const auto c        = finite_subbyte_alg_cfg<Width, S>();
            ...
            const auto signed_code
                = static_cast<std::int64_t>(p) - static_cast<std::int64_t>(c.n);
        ...
        FPSAN_HOST_DEVICE Value<float, S, C> packed_field_widen(std::uint32_t field)
        {
            ...
            {
                static_assert(always_false<std::integral_constant<int, Width>>,
                              "FPSan semantics required");
            }
        }
