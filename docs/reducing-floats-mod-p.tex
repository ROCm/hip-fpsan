...
in the Laurent order
\[
  \Z[\alpha,\alpha^{-1}].
\]
The explicit dyadic localization no longer needs separate notation, because
\(1/2=\alpha^{-4}\).  This remains true regardless of whether the format has a
binary mantissa, and regardless of the mantissa width: mantissa bits introduce
only integer coefficients and powers of \(2\), which are already powers of
\(\alpha\).  Negative quarter-exponents introduce no new primes in the
denominator, because
\[
  \alpha^{-1}=\alpha^3/2,
\]
