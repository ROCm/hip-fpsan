...
For the Field-family prime moduli, the implementation uses the quadratic
residue split as a canonical sign convention.  A quadratic residue modulo \(p\)
is a nonzero residue that is a square: \(x\) is a quadratic residue when
\(x=y^2\) for some nonzero \(y\in\F_p\).  We call exactly these finite nonzero
residues positive.  Zero is not positive; neither are the infinity and NaN
sentinels.  Since the Field primes satisfy \(p\equiv 3\pmod 4\), \(-1\) is not
a square, so exactly one of \(x\) and \(-x\) is positive for every finite
nonzero \(x\).  This gives a canonical absolute value
\[
  |x|_{\mathrm{qr}} =
  \begin{cases}
    x, & x\text{ a quadratic residue},\\
    -x, & \text{otherwise},
  \end{cases}
\]
with \(|x|_{\mathrm{qr}}=|-x|_{\mathrm{qr}}\) for finite nonzero \(x\).  In
terms of the Legendre symbol \(\chi(x)\in\{\pm1\}\), this is
\(|x|_{\mathrm{qr}}=\chi(x)x\) for finite nonzero \(x\).  The extension to the
special cases is by the same implementation rule: since zero and the sentinels
are not positive, \texttt{abs} negates them, and negation fixes them in this
model.  Thus \(\texttt{abs}(0)=0\), \(\texttt{abs}(\infty)=\infty\), and
\(\texttt{abs}(\mathrm{NaN})=\mathrm{NaN}\).

This sign convention is compatible with multiplication on finite nonzero
residues.  Products of two positive residues and products of two negative
residues are positive, while a positive times a negative is negative:
\[
...
non-residues.  Equivalently, \(\chi(xy)=\chi(x)\chi(y)\).  Consequently
\[
  |xy|_{\mathrm{qr}}=|x|_{\mathrm{qr}}\,|y|_{\mathrm{qr}}
\]
for finite nonzero \(x,y\).  Every finite nonzero square is positive, and the
Field square-root power map \(S\) from Constraint~\ref{con:sqrt} is exact on
squares in the canonical sense:
\[
...
\]
Here \(Q\) still means only finite nonzero quadratic residues; zero, infinities,
NaNs, and finite non-residues all lie outside \(Q\).  The non-strict comparison
is not the same relation as the strict comparison.  It is defined in the usual
C++ way, \(a\le b\) iff not \(b<a\), hence
\[
...
finite non-residue \(n\), while neither \(0<n\) nor \(n<0\) holds.
...
finite nonzero values become positive after \texttt{abs}.
...
whenever any finite \(a_i\) is nonzero.  Likewise, if a routine compares
candidate denominators represented as exact squares \(r_i^2\), the nonzero
candidates are positive, the chosen square root is the canonical
\(|r_i|_{\mathrm{qr}}\), and division by the selected value is ordinary field
division rather than an opaque tag or a zero fallback.  This is the algebraic
analogue of choosing a largest pivot or largest \(L^2\)-norm denominator: the
selection is deterministic and preserves the square/root/nonzero relationships
that the algorithm needs, while making no claim to reproduce numerical magnitude
order.

Triton FPSan and the composite algebraic modes keep signed-payload fingerprint
ordering instead of this qr convention.
