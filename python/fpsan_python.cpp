...
namespace
{
    ...
    std::string normalize_dtype(nb::handle h)
    {
        ...
        if(n == "float" || n == "float64" || n == "double" || n == "np_float64"
           || n == "numpy_float64")
            return "float64";
        if(n == "float32" || n == "single" || n == "f32" || n == "np_float32"
           || n == "numpy_float32")
            return "float32";
    ...
    std::string semantics_suffix(Semantics s)
    {
        return fpsan::semantics_name(s);
    }
    ...
    bool same_template(Semantics a, Semantics b)
    {
        return a == b;
    }

    template <class F>
    std::uint64_t storage_bits(F v)
    {
        return static_cast<std::uint64_t>(v.to_storage_bits());
    }

    template <class F>
    std::uint64_t payload(F v)
    {
    ...
    template <class F>
    void bind_value_ops(nb::class_<F>& cls)
    {
        cls.def(
    ...
    nb::object bind_value(nb::module_& m, const char* py_name, const char* dtype)
    {
        ...
        auto cls = nb::class_<F>(m, py_name)
                       ...
                       .def(
                           "__init__",
                           [](F* self, double v) { new(self) F(from_double<FT>(v)); },
                           nb::arg("value"))
                       .def_static("from_storage_bits",
                                   [](std::uint64_t bits) {
                                       return F::from_storage_bits(
                                           static_cast<typename F::bits_type>(bits));
                                   })
    ...
    template <class FT>
    void add_dtype(nb::module_& m, const char* dtype, const char* dtype_prefix)
    {
...
NB_MODULE(_fpsan, m)
{
    ...
    m.def(
        ...
        nb::arg("dtype"),
        nb::arg("semantics"),
        nb::arg("conversions") = Conversions::Explicit,
