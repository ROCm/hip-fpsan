...
template <class T> T sample(float x) {
...
template <class V> std::uint64_t storage_u64(const V &v) {
...
template <class Fn> double time_once(Fn &&fn, std::size_t iters, std::uint64_t &checksum) {
...
template <class Fn>
Result run_case(const char *group, const char *case_name, const char *semantics, Fn &&fn,
                std::size_t min_iters, std::size_t max_iters, double target_ms) {
...
void bench_arith(std::vector<Result> &results, const char *wtag) {
  ...
  auto add_mul = [a_values, b_values, c_values](std::size_t iters) -> std::uint64_t {
    ...
    for (std::size_t i = 0; i < iters; ++i) {
      x = (x + a_values[i & 1023]) * b_values[(i + 17) & 1023] - c_values[(i + 73) & 1023];
      if ((i & 255) == 0)
...
void bench_cast(std::vector<Result> &results, const char *case_name, std::size_t min_iters,
                std::size_t max_iters, double target_ms) {
  ...
  results.push_back(
      run_case("cast", case_name, fpsan::semantics_name(S), fn, min_iters, max_iters, target_ms));
}

template <fpsan::Semantics S> void bench_casts_for_semantics(std::vector<Result> &results) {
  constexpr bool expensive = fpsan::detail::has_multiplicative_field_casts(S);
  bench_cast<S, fpsan::fp8_e4m3, float>(results, "fp8->f32", 1024, expensive ? 1u << 20 : 1u << 25,
                                        60.0);
  bench_cast<S, float, fpsan::fp8_e4m3>(results, "f32->fp8", 1024, expensive ? 1u << 20 : 1u << 25,
                                        60.0);
  bench_cast<S, _Float16, float>(results, "f16->f32", 1024, expensive ? 1u << 20 : 1u << 25, 60.0);
  bench_cast<S, float, _Float16>(results, "f32->f16", 1024, expensive ? 1u << 20 : 1u << 25, 60.0);
  bench_cast<S, float, double>(results, "f32->f64", 16, expensive ? 1u << 17 : 1u << 25, 60.0);
  bench_cast<S, double, float>(results, "f64->f32", 16, expensive ? 1u << 17 : 1u << 25, 60.0);
}
...
void print_results(const std::vector<Result> &results) {
  ...
  for (const Result &r : results) {
    ...
    std::printf("CPU,%s,%s,%s,%.6f,%.6f,%zu,%016llx\n", r.group.c_str(), r.case_name.c_str(),
                r.semantics.c_str(), r.ns_per_iter, r.ns_per_iter / baseline, r.iters,
