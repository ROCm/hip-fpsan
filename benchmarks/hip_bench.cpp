...
template <class T> FPSAN_HOST_DEVICE T sample(float x) {
...
template <class V> FPSAN_HOST_DEVICE unsigned long long storage_u64(const V &v) {
...
__global__ void addmul_preembed_kernel(
    ...
    unsigned input_mask, unsigned long long *out, int iters) {
  ...
  for (int i = 0; i < iters; ++i) {
    const unsigned ia = (static_cast<unsigned>(tid) + 17u * static_cast<unsigned>(i)) & input_mask;
    const unsigned ib = (static_cast<unsigned>(tid) + 29u * static_cast<unsigned>(i)) & input_mask;
    const unsigned ic = (static_cast<unsigned>(tid) + 43u * static_cast<unsigned>(i)) & input_mask;
...
struct OpExp {
  template <class V> __device__ static V apply(V v) { return fpsan::exp(v); }
};
struct OpLog {
  template <class V> __device__ static V apply(V v) { return fpsan::log(v); }
};
struct OpSin {
  template <class V> __device__ static V apply(V v) { return fpsan::sin(v); }
};
struct OpCos {
  template <class V> __device__ static V apply(V v) { return fpsan::cos(v); }
};
...
__global__ void
cast_kernel(const typename fpsan::Value<FromFT, S, fpsan::Conversions::Explicit>::bits_type *in,
            unsigned input_mask, unsigned long long *out, int iters) {
  using From = fpsan::Value<FromFT, S, fpsan::Conversions::Explicit>;
  const int tid = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned long long checksum = 0;
  for (int i = 0; i < iters; ++i) {
    const unsigned idx = (static_cast<unsigned>(tid) + 33u * static_cast<unsigned>(i)) & input_mask;
...
unsigned long long checksum_device_output(unsigned long long *d_out, int count) {
  ...
  HIP_CHECK(hipMemcpy(h_out.data(), d_out, h_out.size() * sizeof(unsigned long long),
...
Result run_gpu_case(const char *group, const char *case_name, const char *semantics, int blocks,
                    int threads_per_block, int iters, Launch &&launch) {
...
void run_arith_for_semantics(std::vector<Result> &results, const char *wtag) {
  ...
  for (unsigned i = 0; i < input_count; ++i) {
    ...
    h_a[i] = V(sample<FT>(0.25f + jitter * 1.0e-4f)).to_storage_bits(); // (0,~0.35], ok sqrt/log
  ...
  results.push_back(run_gpu_case(
      "arithmetic", nm("add/mul/sub").c_str(), s, 256, 256, 1024, [=](unsigned long long *out) {
        addmul_preembed_kernel<FT, S><<<256, 256>>>(d_a, d_b, d_c, input_count - 1, out, 1024);
      }));
  results.push_back(run_gpu_case(
...
template <fpsan::Semantics S, class FromFT, class ToFT>
void run_cast_case(std::vector<Result> &results, const char *case_name, int blocks,
                   int threads_per_block, int iters) {
  using From = fpsan::Value<FromFT, S, fpsan::Conversions::Explicit>;
  ...
  results.push_back(
      run_gpu_case("cast", case_name, fpsan::semantics_name(S), blocks, threads_per_block, iters,
...
template <fpsan::Semantics S> void run_casts_for_semantics(std::vector<Result> &results) {
  ...
  run_cast_case<S, float, double>(results, "f32->f64", expensive ? 16 : 256, 256,
                                  expensive ? 1 : 256);
  run_cast_case<S, double, float>(results, "f64->f32", expensive ? 16 : 256, 256,
                                  expensive ? 1 : 256);
}

void print_results(const char *device_name, const char *arch_name,
  ...
  for (const Result &r : results) {
    ...
    std::printf("HIP,%s,%s,%s,%.6f,%.6f,%d,%d,%016llx\n", r.group.c_str(), r.case_name.c_str(),
                r.semantics.c_str(), r.ns_per_iter, r.ns_per_iter / baseline, r.threads, r.iters,
                r.checksum);
  }
}
