...
namespace fpsan_test
{
    ...
    constexpr bool flavor_is_value_faithful()
    {
        (void)sizeof(FT);
    ...
    // reduce exp to a tag.
    template <class FT, fpsan::Semantics S>
    constexpr bool flavor_has_exp_homomorphism()
    {
    ...
    template <class FT, fpsan::Semantics S>
    constexpr bool flavor_has_cast_homomorphism()
    {
        if constexpr(S == fpsan::Semantics::Triton)
            return true;
    ...
    template <class FT, fpsan::Semantics S>
    constexpr bool flavor_has_cbrt()
    {
        if constexpr(fpsan::detail::is_algebraic_semantics(S)
                     && !fpsan::detail::has_fast_field_ops(S))
