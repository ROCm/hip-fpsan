...
#define FPSAN_DEFINE_SUB6_UNPACK_CANONICAL_TEST(CASE, WRAP, DST, LABEL)                     \
    ...
    {                                                                                       \
        if(!have_device())                                                                  \
            GTEST_SKIP() << "no HIP device";                                                \
        const std::uint8_t codes[32]                                                        \
            = {0x00u, 0x01u, 0x1Fu, 0x20u, 0x21u, 0x2Au, 0x30u, 0x3Fu, 0x02u, 0x07u, 0x10u, \
               0x18u, 0x22u, 0x2Fu, 0x3Eu, 0x15u, 0x04u, 0x08u, 0x0Fu, 0x11u, 0x1Eu, 0x23u, \
               0x31u, 0x3Du, 0x05u, 0x09u, 0x12u, 0x19u, 0x24u, 0x2Cu, 0x38u, 0x3Cu};       \
        ...
        for(int i = 0; i < 32; ++i)                                                         \
        {                                                                                   \
            const auto want = fpsan::cast<DST>(                                             \
                fpsan_test::canonical_subbyte_widen<6, S, kCC>(codes[i]) * VF{scale});      \
            ...
            EXPECT_EQ(got[32 + i], static_cast<unsigned>((want + want).fpsan_payload()))    \
                << LABEL << " follow-on add elem " << i << " code=0x" << std::hex           \
                << unsigned(codes[i]);                                                      \
            (void)sizeof(Out);                                                              \
        }                                                                                   \
        (void)hipFree(dIn);                                                                 \
...
TEST(CvtScalef32Fp6Wrap, FpsanPackDivideInterleave)
{
    ...
    fpsan_test::for_each_fpsan_semantics([&](auto sem) {
        ...
        for(int k = 0; k < 16; ++k)
        {
            ...
            unsigned refHi
                = fpsan_test::canonical_subbyte_narrow_code<6>(VF{in[16 + k]} / VF{scale});
