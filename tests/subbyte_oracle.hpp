...
{
    template <int Width>
    std::int32_t sign_extend_subbyte(std::uint32_t field)
    {
    ...
    fpsan::detail::AlgConfig finite_subbyte_alg_cfg()
    {
    ...
    std::uint32_t canonical_subbyte_code_from_residue(std::uint64_t p)
    {
        ...
        const auto signed_code
            = static_cast<std::int64_t>(residue) - static_cast<std::int64_t>(c.n);
        return static_cast<std::uint32_t>(signed_code) & mask;
    }

    template <int Width>
    constexpr std::uint32_t subbyte_positive_max_code()
    {
        return (std::uint32_t{1} << (Width - 1)) - 1u;
    }

    template <class DstFT, int Width, fpsan::Semantics S, fpsan::Conversions C>
    typename fpsan::Value<DstFT, S, C>::bits_type
        canonical_subbyte_widen_payload(std::uint32_t field)
    {
