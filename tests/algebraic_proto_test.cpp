...
template <class FT>
static u64 emb(const AlgConfig& c, FT v)
{
    return alg_embed1(c, raw_bits(v));
}
...
static void test_width(const char* name, AlgVariant field, AlgVariant exp)
{
    ...
    for(int i = -8; i <= 8; ++i)
        ...
        {
            hadd
                += alg_add1(cf, emb<FT>(cf, (FT)i), emb<FT>(cf, (FT)j)) == emb<FT>(cf, (FT)(i + j));
            hmul
                += alg_mul1(cf, emb<FT>(cf, (FT)i), emb<FT>(cf, (FT)j)) == emb<FT>(cf, (FT)(i * j));
        }
    ...
    for(int k = 0; k < 5000; ++k)
    {
        ...
        seed  = seed * 6364136223846793005ull + 1;
        u64 b = (seed >> 11) % ce.n;
        e2 += alg_exp2_1(ce, alg_add1(ce, a, b))
              == alg_mul1(ce, alg_exp2_1(ce, a), alg_exp2_1(ce, b));
